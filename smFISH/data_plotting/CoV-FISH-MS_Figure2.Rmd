...
## gRNA line profiles 
```{r}
line_profile_data <- read_csv("./Data/CoV-FISH-07_N-protein/N-protein_gRNA_line-profiles.csv")
...
```{r}
# Import data
N_overlap <- read_csv("./Data/Figure2/p_NProtein-overlap.csv")

# Get mean labels
N_overlap_anno <- N_overlap %>%
  group_by(species) %>%
  summarise(mean = mean(p_NCed)) %>%
  ungroup() %>%
  mutate(label = paste0(round(mean, 1), "%"))

# Statistics
N_overlap %>% group_by(species) %>% shapiro_test(p_NCed) # normal
star <- N_overlap %>% t_test(p_NCed ~ species) %>% add_significance() %>% pull(p.signif)
pval <- N_overlap %>% t_test(p_NCed ~ species) %>% add_significance() %>% pull(p) 
stat_anno <- paste0(star, "\np=", pval)

# Plot 
N_overlap %>%
  mutate(species = fct_rev(species)) %>%
  ggplot(aes(x = species, y = p_NCed, fill = species)) +
  ...
  geom_label(data = N_overlap_anno,
             aes(x = species, y = mean + 15, label = label), 
  ...
  annotate(geom = "text", x = 1.5, y = 90, label = stat_anno, size = 2, colour = "gray10") +
  coord_flip() -> plot2
