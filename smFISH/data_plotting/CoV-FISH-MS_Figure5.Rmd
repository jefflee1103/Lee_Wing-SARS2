...
## 24hpi
nndist_VIC_24h <- read_csv("./Data/Figure5/Vero_Vic_24h_spatial_nn-distances.csv") %>%
  dplyr::select(c("Observed" = nn_distances_data, "Random" = nn_distances_sim)) %>%
  pivot_longer(everything(), names_to = "data_type", values_to = "nn_distance") %>%
  filter(!is.na(nn_distance)) %>%
  mutate(nn_distance = nn_distance * 0.45)

nndist_B117_24h <- read_csv("./Data/Figure5/Vero_B117_24h_spatial_nn-distances.csv") %>%
  dplyr::select(c("Observed" = nn_distances_data, "Random" = nn_distances_sim)) %>%
  pivot_longer(everything(), names_to = "data_type", values_to = "nn_distance") %>%
  filter(!is.na(nn_distance)) %>%
  mutate(nn_distance = nn_distance * 0.45)
...
## 8hpi
nndist_VIC_8h <- read_csv("./Data/Figure4/Spatial-randomness_Vero_nn_distances.csv") %>%
  dplyr::select(c("Observed" = nn_distances_data, "Random" = nn_distances_sim)) %>%
  pivot_longer(everything(), names_to = "data_type", values_to = "nn_distance") %>%
  filter(!is.na(nn_distance)) %>%
  mutate(nn_distance = nn_distance * 0.65)

nndist_B117_8h <- read_csv("./Data/Figure5/Vero_B117_8h_spatial_nn-distances.csv") %>%
  dplyr::select(c("Observed" = nn_distances_data, "Random" = nn_distances_sim)) %>%
  pivot_longer(everything(), names_to = "data_type", values_to = "nn_distance") %>%
  filter(!is.na(nn_distance)) %>%
  mutate(nn_distance = nn_distance * 0.65)
...
```{r}
nndist_VIC_8h %>% filter(data_type == "Observed") %>% pull(nn_distance) %>% e1071::skewness()

nn_distance_vector <- nndist_B117_24h %>% filter(data_type == "Observed") %>% pull(nn_distance)
2*(0.55/0.33)*(nn_distance_vector %>% mean()) / (sqrt(79925002*2/(length(nn_distance_vector))))

nn_distance_vector <- nndist_VIC_24h %>% filter(data_type == "Observed") %>% pull(nn_distance)
2*(0.55/0.33)*(nn_distance_vector %>% mean()) / (sqrt(28925002*2/(length(nn_distance_vector))))
